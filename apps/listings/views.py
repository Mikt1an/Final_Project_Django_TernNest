...
class ListingDetailView(RetrieveUpdateDestroyAPIView,):
    ...
    def retrieve(self, request, *args, **kwargs,):
        ...
        if created:
            current_count = getattr(listing, "views_count_value", None,)

            if current_count is not None:
                listing.views_count_value = current_count + 1
...
class ListingImageListCreateView(ListCreateAPIView,):
    ...
    def get_listing(self):
        ...
        listing = get_object_or_404(queryset, pk=self.kwargs["listing_pk"],)
...
class ListingImageDetailView(RetrieveUpdateDestroyAPIView,):
    serializer_class = ListingImageSerializer

    permission_classes = IsListingImageOwnerOrReadOnly,

    def get_queryset(self):
        queryset = (ListingImage.objects.select_related("listing", "listing__owner",).filter(listing_id=self.kwargs["listing_pk"],))

        if self.request.method in SAFE_METHODS:
            if self.request.user.is_authenticated:
                queryset = queryset.filter(Q(listing__is_active=True) | Q(listing__owner=(self.request.user)))

            else:
                queryset = queryset.filter(listing__is_active=True,)
...
class FavoriteListCreateView(ListCreateAPIView,):
    ...
    def get_queryset(self):
        return Favorite.objects.filter(user=self.request.user,).select_related("listing", "listing__owner",)
...
class ListingViewHistoryView(ListAPIView,):
...
class PopularSearchListView(APIView,):
